...
export default function MeDashboardPage() {
  const profile = mockProfiles.find(p => p.id === 'me')!;
  const today = new Date().toISOString().split('T')[0];
  
  const todaysLogs = mockMealLogs.filter(log => log.date === today && log.person === 'me');
  ...
  const weekData = Array.from({ length: 7 }, (_, i) => {
    ...
    const dayLogs = mockMealLogs.filter(log => log.date === dateStr && log.person === 'me');
    return {
      day: format(date, 'EEE'),
      date: dateStr,
      ...
      <div className="space-y-6">
        ...
        <SectionCard title="This Week" subtitle="Daily calorie intake">
          <div className="flex items-end justify-between gap-2 h-32">
            {weekData.map(day => {
              const maxCal = profile.dailyTargets?.calories || 2200;
